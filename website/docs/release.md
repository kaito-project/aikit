...
If a mutable alias or GitHub Latest needs repair after a transient failure, run **Reconcile release latest** from `main`. It recomputes the target from current immutable tags and successful publisher runs; it never trusts the triggering run's version. This repair workflow applies to releases produced by the protected attempt-bound publishers. Legacy runs lack those signed candidate tags and require a separate manual provenance review.
