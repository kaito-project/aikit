...
5. The tag starts the artifact and runner-image publishing workflows. They publish only versioned `vX.Y.Z` artifacts. After each workflow completes, a trusted default-branch reconciler selects the highest stable version whose complete workflow succeeded and updates the applicable `latest` aliases. The app reconciler also assigns GitHub Latest.
